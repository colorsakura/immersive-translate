...
import { decodeHtmlEntities, fetchWithTimeout, parseJsonResponse, retryOnceOnRateLimit } from './utils';
...
export const googleTranslator: TranslationSource = {
  ...
  async translate(text: string, source: string, target: string, config: SourceConfig): Promise<TranslationResult> {
    ...
    return retryOnceOnRateLimit(async () => {
      ...
      const response = await fetchWithTimeout('https://translation.googleapis.com/language/translate/v2', {
        method: 'POST',
      ...
      if (!translatedText) {
        throw new TranslationError(data.error?.message || 'Google 响应格式错误', 'INVALID_RESPONSE');
      }
